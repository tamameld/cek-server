<html>
<link rel="shortcut icon" href="https://icons.iconarchive.com/icons/iconsmind/outline/128/Plug-In-icon.png">    
</html>
<?php
/* ---------------------------------------------------- */
/* PHP Server Status v1.0
/* 
/* Site: putrairvan.skom.id
/* Email: admin@putrairvan.skom.id
/* Date: 2021-09-20
/* Author: Putra Irvan
/* ---------------------------------------------------- */
...
<div id="page">
	...
	<table>
		<tr>
			<th class="w_logo">Irvan Server Status</th>
			<th class="w_top"><a href="https://putrairvan.skom.id/mesin/index.zip">Unduh Script</a></th>
		</tr>
	</table>
...
<table>
  ...
  <tr>
    ...
    <td width="18%">
		...
		$disFuns=get_cfg_var("disable_functions");
		?>
    <?php echo (false!==eregi("phpinfo",$disFuns))? '<font color="red">Г—</font>' :"<a href='$phpSelf?act=phpinfo' target='_blank'>PHPINFO</a>";?>
    </td>
  ...
  <tr>
    <td>Cookieпјљ</td>
    <td><?php echo isset($_COOKIE)?'<font color="green">в€љ</font>' : '<font color="red">Г—</font>';?></td>
   ...
   <tr>
    <td>SMTPпјљ</td>
    <td><?php echo get_cfg_var("SMTP")?'<font color="green">в€љ</font>' : '<font color="red">Г—</font>';?></td>
    <td>SMTPпјљ</td>
    <td><?php echo get_cfg_var("SMTP")?get_cfg_var("SMTP"):'<font color="red">Г—</font>';?></td>
  </tr> 
	<tr>
...
<table>
  ...
  <tr>
    ...
    <?php
	    ...
	    }else{echo '<font color="red">Г—</font>';}
	?></td>
    <td>Zlib: </td>
...
<table>
  ...
  <tr>
    ...
    <td width="18%"><?php $zend_version = zend_version();if(empty($zend_version)){echo '<font color=red>Г—</font>';}else{echo $zend_version;}?></td>
    <td width="32%">
<?php
...
?>
    ...
    <td width="18%"><?php if($PHP_VERSION > 2){echo (get_cfg_var("zend_loader.enable"))?'<font color=green>в€љ</font>':'<font color=red>Г—</font>';} else{if(function_exists('zend_optimizer_version')){	echo zend_optimizer_version();}else{	echo (get_cfg_var("zend_optimizer.optimization_level")||get_cfg_var("zend_extension_manager.optimizer_ts")||get_cfg_var("zend.ze1_compatibility_mode")||get_cfg_var("zend_extension_ts"))?'<font color=green>в€љ</font>':'<font color=red>Г—</font>';}}?></td>
  </tr>
  <tr>
    <td>eAccelerator</td>
    <td><?php if((phpversion('eAccelerator'))!=''){echo phpversion('eAccelerator');}else{ echo "<font color=red>Г—</font>";} ?></td>
    <td>ioncube</td>
    <td><?php if(extension_loaded('ionCube Loader')){   $ys = ioncube_loader_iversion();   $gm = ".".(int)substr($ys,3,2);   echo ionCube_Loader_version().$gm;}else{echo "<font color=red>Г—</font>";}?></td>
  </tr>
  <tr>
    <td>XCache</td>
    <td><?php if((phpversion('XCache'))!=''){echo phpversion('XCache');}else{ echo "<font color=red>Г—</font>";} ?></td>
    <td>APC</td>
    <td><?php if((phpversion('APC'))!=''){echo phpversion('APC');}else{ echo "<font color=red>Г—</font>";} ?></td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td>SQLite: </td>
    <td><?php if(extension_loaded('sqlite3')) {$sqliteVer = SQLite3::version();echo '<font color=green>в€љ</font>гЂЂ';echo "SQLite3гЂЂVer ";echo $sqliteVer[versionString];}else {echo isfun("sqlite_close");if(isfun("sqlite_close") == '<font color="green">в€љ</font>') {echo "&nbsp;пјљ ".@sqlite_libversion();}}?></td>
    <td>Hyperwave: </td>
